#let template(
  ...
  if (short-citation == auto and authors.len() == 1) {
    short-citation = authors.at(0).name + year
  ...
  set page(
    ...
    header: locate(loc => {
      if(loc.page() == 1) {
        ...
        return align(right, text(size: 8pt, fill: gray, "Lecture 20 minutes"))
      } else {
  ...
  place_margin("left", 
    box(width: 27%, {
      if (kind != none) {
        ...
        text(12pt, fill: theme, weight: "semibold", smallcaps(kind))
        parbreak()
        v(1em)
        text(8pt, fill: black, weight: "semibold", "Numéro " + str(numero))
      }
      if (dates != none) {
        let formatted-dates
        ...
        grid(columns: (40%, 60%), gutter: 7pt,
          ..dates.zip(range(dates.len())).map((formatted-dates) => {
            ...
            return (
              text(size: 7pt, fill: theme, weight: weight, "Publié le "),
              text(size: 7pt, date_fr)
            )
          }).flatten()
        )
      }
  ...
  if (type(abstract) == "content") {
    abstracts = (title: "Introduction", content: abstract)
  } else {
    abstracts = abstract
  }
  ...
  box(inset: (top: 16pt, bottom: 16pt), stroke: (top: 1pt + gray, bottom: 1pt + gray), {
      ...
      text(fill: theme, weight: "semibold", size: 9pt, abstracts.title)
      parbreak()
      abstracts.content
  })
  if (keywords.len() > 0) {
